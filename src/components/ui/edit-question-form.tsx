...
export function EditQuestionForm({
  ...
  useEffect(() => {
    if (questionInfo) {
      // Set question text
      setQuestion(questionInfo.questionName || "")

      // Determine question type
      const type = questionInfo.type === "freeResponse" ? "freeResponse" : "multipleChoice"
      ...
      if (type === "multipleChoice" && questionInfo.answerChoices.length > 0) {
        ...
        const formattedOptions = questionInfo.answerChoices.map((choice, index) => {
          ...
          const isCorrect = questionInfo.correctAnswer.some((answer) => {
            ...
            const answerIndex = Number.parseInt(answer.split("option-")[1]) - 1
      ...
      } else if (type === "freeResponse" && questionInfo.correctAnswer.length > 0) {
        // Set free response answer
        setFreeResponseAnswer(questionInfo.correctAnswer[0] || "")
      }
    }
  }, [questionInfo])
  ...
  const handleOptionCorrectChange = (id: string, isCorrect: boolean) => {
    setOptions(options.map((option) => (option.id === id ? { ...option, isCorrect } : option)))
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updatedQuestion: Question = {
        ...
        type: questionType,
        answerChoices: questionType === "multipleChoice" ? options.map((opt) => opt.text) : [],
        correctAnswer:
          questionType === "multipleChoice"
            ? options
                .filter((opt) => opt.isCorrect)
                .map((opt) => opt.id) // Use option.id instead of option.text
            : [freeResponseAnswer],
      }
  ...
  return (
    <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
      {error && (
        <div className="bg-red-900/20 border border-red-800 text-red-300 px-3 py-2 rounded-md text-sm">{error}</div>
      )}
      ...
      <div className="grid gap-2">
        ...
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="multipleChoice" id="multipleChoice" className="text-white border-white" />
            <Label htmlFor="multipleChoice" className="text-zinc-300">
              Multiple Choice
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="freeResponse" id="freeResponse" className="text-white border-white" />
      ...
      {questionType === "multipleChoice" ? (
        <div className="grid gap-3">
          ...
          <p className="text-xs text-zinc-500 mt-1">
            Check the boxes next to correct answers. You can select multiple correct answers.
          </p>
        </div>
      ) : (
        <div className="grid gap-2">
      ...
      <div className="flex justify-end gap-2 mt-4">
        <Button
          ...
          disabled={
            ...
            !question ||
            (questionType === "multipleChoice"
              ? options.some((opt) => !opt.text) || !options.some((opt) => opt.isCorrect)
              : !freeResponseAnswer)
          }
